...
rbind(
  get_market_value(port) %>% 
    filter(last_updated >= (max(last_updated) - days(1) - hours(1))) %>%
    mutate(cash_change = c(0, diff(cash)),
           net_investment_value = investments_value + cash_change) %>% 
    summarise_at(c("cash", "investments_value", "net_investment_value", "net_value"),
                 .funs = funs(tail(., 1)/head(., 1) - 1)),
  get_market_value(port) %>% 
    filter(last_updated >= (max(last_updated)- weeks(1) - hours(1))) %>%
    mutate(cash_change = c(0, diff(cash)),
           net_investment_value = investments_value + cash_change) %>% 
    summarise_at(c("cash", "investments_value", "net_investment_value", "net_value"),
                 .funs = funs(tail(., 1)/head(., 1) - 1)),
  get_market_value(port) %>% 
    filter(last_updated >= (max(last_updated) - months(1) - hours(1))) %>%
    mutate(cash_change = c(0, diff(cash)),
           net_investment_value = investments_value + cash_change) %>% 
    summarise_at(c("cash", "investments_value", "net_investment_value", "net_value"),
                 .funs = funs(tail(., 1)/head(., 1) - 1)),
  get_market_value(port) %>% 
    ...
    summarise_at(c("cash", "investments_value", "net_investment_value", "net_value"),
                 .funs = funs(tail(., 1)/head(., 1) - 1)),
  get_market_value(port) %>% 
    filter(last_updated >= (max(last_updated) - years(1) - hours(1))) %>%
    mutate(cash_change = c(0, diff(cash)),
           net_investment_value = investments_value + cash_change) %>% 
    summarise_at(c("cash", "investments_value", "net_investment_value", "net_value"),
                 .funs = funs(tail(., 1)/head(., 1) - 1))
