...
# R packages
packages <- c("a2munge", "a2modeler", "a2charter", "dplyr", "tidyr", "readxl", "tibble", "purrr", "ggplot2", "stringr", "lubridate", "sparklyr", "DT", "rlang")
...
destfile <- paste(c("Shiller-", as.character(Sys.Date()), ".xls"), collapse = "")
...
df <- clean %>% 
  dplyr::mutate(ftr_ret_1yr = lead(price, 12) / price - 1,
                ...
                gs10_real_d_1yr = gs10_real / lag(gs10_real, 12) - 1,
                gs10_real_d_5yr = gs10_real / lag(gs10_real, 60) - 1,
...
r1 <- sdf %>% 
  regressor(.,
            "ftr_ret_1yr",
            name = "mkt-shiller-5yr-forecast",
            uid = sparklyr::random_string("regressor"),
            version = "1.0.0",
            desc = "Regression model to predict 5 year market return using Shiller CAPE metric",
            scientist = "Haarstick",
  ...
  #           pipe = feature_pipe,
  #           param_map = list(max_iter = c(50, 100, 200),
  #                            max_depth = c(2, 5),
  #                            subsampling_rate = c(.75, 1)),
  #           uid = "gbt",
  #           desc = "GBT with moderate tuning") %>%
  train_models() %>% 
  set_final_model("best", reevaluate = TRUE, refit = FALSE)
...
```{r execution, eval=TRUE, ref.label=c('r-config', 'r-setup', 'spark-config', 'spark-setup',  'data-import', 'data-prep', 'data-transform', 'data-split','spark-import', 'feature-pipeline', 'regressor', 'predicted')}
